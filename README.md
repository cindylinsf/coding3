...
## Results & Evaluation
I think one of the key elements that that drew me to machine learning is its "randomness" where we have no real control over what is generated. We can set parameters, but we can overtrain the model and things can happen. There is no exact parameters or the exact number of iterations until we test and experiment with it. 

Like Zylinska who was curious about what AI would do to the source material, while intentionally serving as both a curator of randomness and analyst in the creation process, I too feel the same when using machine learning as a tool for creating artistic work. To me, the idea of using a machine learning model is akin to using film to create photographic work. I didn't learn how to work with film until well into my 30s. So while I understood the exposure triangle, my results may not be precisely as intended. Working with film is also significantly more costly and laborious than working with digital photography. Not only do films cost a lot to purchase nowadays, but they also expire, which will impact the results (but can also turn out to be amazing! Some photographers prefer working with expired films.). Development is also costly and laborious. You can ruin months of work in a second if you accidentally expose the film or go too heavy-handed with the developing chemicals. There is no real control and the result may not be precise, but that's the magic of capturing that fleeting moment with a camera and working with film as a medium. It is an apparatus for making work, just like a machine learning model.

So I was disappointed by the final script generated by ChatGPT4o. I thought the final script, while cleaned up and read nicely, reads a lot like the Season 1 ending of Netflix's _The Stranger Things_. Instead of evil birds serving as rich metaphors ranging from war to female rage and desire and as a "Freudian three-ring circus"[7], in the new draft, all the interesting elements that made Hitchcock's _The Birds_ a classic, seem all flew out of the window. Instead, it was a trite, typical regurgitation of Hollywood plot, devoid of any personalities.
